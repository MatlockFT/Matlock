(() => {
  ...
  function ensurePreviewMediaIdentity(figure) {
    if (!figure) return '';
    const current = figure.dataset.writerMediaId || '';
    ...
    const blockMatch = index >= 0 ? blocks[index] : null;
  ...
  function applyPreviewMediaLayout(figure, layout) {
    ...
    [...figure.classList].forEach(name => {
      if (new RegExp('^' + base + '--(?:break|wrap|left|center|right|small|medium|large|full)
...
).test(name)) figure.classList.remove(name);
    });
...
window.addEventListener('matlock-writer:auth-expired', () => setPublishingControls(false));
...
).test(name)) node.classList.remove(name);
    });
  ...
  function persistPreviewMediaLayout(figure) {
    const id = ensurePreviewMediaIdentity(figure);
    if (!id) {
      showToast('Could not match that media block back to the article source.', 5000);
      return false;
    }

    const escapedId = id.replace(/[.*+?^$()|[\]\\]/g, '\\  function videoControlIcon(kind) {');
    const openingPattern = new RegExp('<figure\\b[^>]*data-writer-media-id=["\\']' + escapedId + '["\\'][^>]*>', 'i');
    const match = bodyEditor.value.match(openingPattern);
    if (!match) return false;

    const replacement = rewriteMediaOpeningTag(match[0], figure);
    bodyEditor.value = bodyEditor.value.replace(openingPattern, replacement);
    bodyEditor.dispatchEvent(new Event('input', { bubbles:true }));
    return true;
  }
  ...
  function hydratePreviewMediaTools() {
    ...
    previewContent.querySelectorAll('figure.article-inline-image, figure.article-inline-video').forEach(figure => {
      const layout = readPreviewMediaLayout(figure);
      applyPreviewMediaLayout(figure, layout);
