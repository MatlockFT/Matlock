...
The backtest candidate universe is broader than the live public dataset. The refresh builds `assets/data/matchmaker/historical-universe.json` from the UFCStats fighter directory and verified UFC/TUF/Road-to-UFC fight ledger. Fighters that still exist in the public Matchmaker data retain their canonical IDs; historical-only fighters use stable source-native UFCStats identities. This historical universe is loaded only by offline validation and is not exposed as additional live Matchmaker candidates.
...
Every sampled historical case receives an explicit diagnostic classification. Cases that cannot fairly test the engine are separated from scoring: an eventual opponent with no pre-cutoff UFC history is a `future-newcomer-booking`; an eventual bout that requires the subject or opponent to make a future weight-class move is a future division-change case; known booking conflicts and unresolved source coverage are also excluded. Future fight information is used only after candidate scoring to classify the case and never to alter the recommendation ranking.

The clean evaluation set therefore contains only cases where the subject and actual opponent can both be reconstructed in the same historical division, the opponent was available under known-at-the-time evidence, and the engine's hard eligibility rules allow the matchup. Those cases are then separated into Top-3 hits, near ranking misses, deep ranking misses and candidate omissions. Hard-rule disagreements remain a separate review bucket because an actual UFC booking may expose an engine rule that is too strict, or it may simply be a short-notice, promotional or otherwise unusual UFC booking.
...
- Historical-only validation fighters come from source-native UFCStats identities and are never added to the live public Matchmaker roster merely to improve a backtest.
