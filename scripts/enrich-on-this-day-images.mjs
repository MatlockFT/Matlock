...
const USER_AGENT = "MMA-Matlock-OnThisDay-Images/1.0 (+https://matlockfighttalk.com/on-this-day/)";
const REQUEST_TIMEOUT_MS = 20000;
const REQUEST_ATTEMPTS = 3;
const ENRICH_LIMIT = Math.max(1, Number(process.env.OTD_IMAGE_ENRICH_LIMIT || 28));
...
function mmddOrdinal(value) {
    const match = /^(\d{4}-)?(\d{2})-(\d{2})$/.exec(String(value || "").slice(-10));
    if (!match) return 999;
...
function candidateScore(fileTitle, entry) {
    ...
    let score = 0;
    if (/\bposter\b|promotional|promotion\b|promo\b|fight[ _-]?card|event[ _-]?card|official[ _-]?art/i.test(filename)) score += 110;
    if (/cover|key[ _-]?art/i.test(filename)) score += 65;

    const promotion = normalized(entry?.promotion || "");
    if (promotion && filename.includes(promotion.replace(/[^a-z0-9]+/g, " ").trim())) score += 22;

    const numberedEvent = normalized(eventName(entry)).match(/\b(?:ufc|bellator|pfl|rizin|one)\s*(?:fight\s*night\s*)?(\d{1,4})\b/i);
    if (numberedEvent?.[1] && new RegExp(`(?:^|[^0-9])${numberedEvent[1]}(?:[^0-9]|$)`).test(filename)) score += 60;

    let tokenMatches = 0;
    for (const token of meaningfulTokens(entry)) {
        if (filename.includes(token)) tokenMatches += 1;
    }
    score += Math.min(tokenMatches, 6) * 9;

    if (/\b(?:logo|flag|map|icon|symbol|seal|coat[ _-]?of[ _-]?arms|wikidata|commons|location|venue|arena)\b/i.test(filename)) score -= 90;
    if (/headshot|portrait|weigh[ _-]?in|press[ _-]?conference/i.test(filename)) score -= 28;
    if (/screenshot|screen[ _-]?shot/i.test(filename)) score -= 18;
...
async function listPageImages(title) {
    ...
    return Array.isArray(page?.images) ? page.images.map(item => item?.title).filter(Boolean) : [];
}
...
function dimensionAdjustment(item) {
    ...
    if (area >= 1500000) score += 12;
    else if (area >= 750000) score += 7;
    else if (area >= 350000) score += 3;

    if (ratio >= 0.5 && ratio <= 0.82) score += 12;
    else if (ratio > 0.82 && ratio <= 1.2) score += 7;
    else if (ratio > 1.2 && ratio <= 1.9) score += 5;
    else score += 1;
    return score;
}
...
async function findBetterArtwork(entry) {
    ...
    const candidates = info
        .map(item => ({
            ...item,
            score: (scoreByTitle.get(item.title) ?? 0) + dimensionAdjustment(item)
        }))
...
for (const entry of eligible) {
    try {
        ...
        if (artwork) {
            ...
            cache.entries[entry.autoKey] = {
                strategyVersion: STRATEGY_VERSION,
                status: "pageimage-kept",
                sourceType: "wikipedia-pageimage",
                checkedAt: nowIso,
                wikipediaTitle: wikipediaTitle(entry)
            };
            kept += 1;
...
console.log(`On This Day artwork enrichment: ${eligible.length} checked, ${improved} upgraded, ${kept} page images kept, ${failed} failed, ${applied} cached poster overrides applied.`);
