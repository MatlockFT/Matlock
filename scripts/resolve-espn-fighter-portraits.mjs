...
async function imageWorks(url) {
  if(/(?:piwik|matomo|analytics|tracking|pixel|beacon)/i.test(url||''))return false;
...
for(const [key,name] of byName){
  ...
  const existing=fighters.find(f=>norm(f.name)===key&&f.image&&!/(?:piwik|matomo|analytics|tracking|pixel|beacon)/i.test(f.image));
...
for(const person of fighters){
  if(person.image)continue;const hit=cache[norm(person.name)];if(!hit?.url)continue;
...
if(applied)await fs.writeFile(DATA_PATH,JSON.stringify(data,null,2)+'\n');
console.log(`ESPN name lookup added ${added} portrait(s); applied ${applied} cached portrait(s) to unified event data.`);
