...
for (const filename of (await readdir(postsDirectory)).filter(file => file.endsWith('.md'))) {
  ...
  const scheduledValue = yaml.match(/^publish_at:\s*(.*?)\s*$/m)?.[1]?.replace(/^['"]|['"]$/g, '');

  if (!isDraft || !scheduledValue) continue;
  ...
  const updatedYaml = yaml
    ...
    .replace(/^publish_at:\s*.*(?:\n|$)/m, '');
  const updatedMarkdown = markdown.replace(yaml, updatedYaml.trimEnd());
